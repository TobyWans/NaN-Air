...
class ContractorMenuDL:
    ...
    def add_new_contractor(self, mdl):
        with open(self.contractor_file, 'a', newline='', encoding='utf-8') as contr_file:
            fieldnames = ['contractor','name','profession','phone','opening hours','location','rating']
            writer = csv.DictWriter(contr_file, fieldnames=fieldnames)
            writer.writerow({'contractor':mdl.contractor, 'name':mdl.name, 'profession':mdl.profession, 'phone':mdl.phone, 'opening hours':mdl.opening_hours, 'location':mdl.location, 'rating':mdl.rating})
            contr_file.close()
            return
